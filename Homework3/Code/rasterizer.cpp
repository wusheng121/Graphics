...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    // 把顶点从 Vector3f 转为齐次坐标 Vector4f
    auto v = t.toVector4();

    // ---------- Step 1: 计算 bounding box ----------
    ...
    for (int x = x_min; x <= x_max; x++)
    {
        for (int y = y_min; y <= y_max; y++)
        {
            ...
            if (zp >= depth_buf[index]) continue; // 深度测试
            ...
            Eigen::Vector3f interpolated_shadingcoords =
                (alpha * t.view_pos[0] / v[0].w() +
                 beta  * t.view_pos[1] / v[1].w() +
                 gamma * t.view_pos[2] / v[2].w()) * Z;

            // ---------- Step 6: 调用 fragment shader ----------
